...
import (
	...
	*/
	...
	"github.com/pytorchtw/flashcard-go/genapi/restapi"
	"github.com/pytorchtw/flashcard-go/genapi/restapi/operations"
	//"github.com/pytorchtw/go-simple-github/utils"
...
func Test_setup(t *testing.T) {
	...
	testManager.api = operations.NewFlashcardAPI(swaggerSpec)
...
func Test_GetURLContent(t *testing.T) {
	...
	res, err := http.Post("http://"+testManager.addr+"/api/deck",
		"application/json",
		strings.NewReader(string(bytes)))
	ok(t, err)
	equals(t, 201, res.StatusCode)

	resContent, err := ioutil.ReadAll(res.Body)
	res.Body.Close()
	ok(t, err)
	log.Println(string(resContent))
}
